...
int MaxMoney=0;
Set<Shift> Best={};
Set<Shift> highestValueScheduleForHelper(Set<Shift>& shifts, int maxHours,int timeSum,int moneySum,Set<Shift>& record);//,int& MaxMoney,Set<Shift>& Best);
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    /* TODO: Delete the next few lines and implement this function. */
    Set<Shift>a={};
    Set<Shift>b={};
    //Map<int,Set<Shift>>map;
    //int MaxMoney;
    Set<Shift> Best;
    for(Shift s:shifts){
        b.add(s);
    }
    return highestValueScheduleForHelper(b,maxHours,0,0,a);//,MaxMoney,Best);
}
Set<Shift> highestValueScheduleForHelper(Set<Shift>& shifts, int maxHours,int timeSum,int moneySum,Set<Shift>& record){//,int& MaxMoney,Set<Shift>& Best){
    if(maxHours<0){
        error("less than 0");
        ...
        for(Shift s:shifts){
            ...
            if(timeSum<=maxHours){
                ...
                if(ifnotOverlap){
                    record.add(s);
                    shifts.remove(s);
                    highestValueScheduleForHelper(shifts,maxHours,timeSum,moneySum,record);//,MaxMoney,Best);
                    // if(map.isEmpty()){
                    //     map[moneySum]=record;
                    // }else{
                    //     if(moneySum>map.firstKey()){
                    //         map.clear();
                    //         map[moneySum]=record;
                    //     }
                    // }
